...
# Create a nowcasting visualization using analog retrieval of images from the training data
...
weight <- c(1, 1, rep(1, 29)) %>% as.matrix() %>% t()
...
for (hour in 0:6) {
  ...
  training_paths <- training[idx[1], ]$path
...
writePNG(composite, "experiment/analog_retrieval-image/analog-image.png")
